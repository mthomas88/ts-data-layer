...
export const connectToPostgresql = async () => {
    ...
    const message_queue: { id: ReturnType<typeof crypto.randomUUID>; hex_code: string; hex_type: string; raw_code: number; raw_type: number; message: Buffer, received_at: Date }[] = [];
    ...
    const parseMessage = (message: Buffer) => {
        ...
        message_queue.push({ ...struct, raw_code: codeByte, raw_type: typeByte, hex_type: ret.hex_type, hex_code: ret.hex_code });

        debug(`message [queue_length=${message_queue.length}][id=${ret._id}][raw_type_byte=${ret.raw_type}][raw_code_byte=${ret.raw_code}][hex_type=${ret.hex_type}][hex_code=${ret.hex_code}]`);
